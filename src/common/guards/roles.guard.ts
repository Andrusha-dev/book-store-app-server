...
export class RolesGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean {
    // Зчитуємо ролі, передані в декоратор @Roles()
    const requiredRoles = this.reflector.getAllAndOverride<UserRole[]>(
      ROLES_KEY,
      [context.getHandler(), context.getClass()],
    );

    // Якщо декоратора @Roles() немає — ролі не вимагаються, пускаємо далі
    ...
    if (!user) {
      throw new InternalServerErrorException('RolesGuard used without JwtAuthGuard',);
    }
